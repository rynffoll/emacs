...
(use-package magit
  :preface
  (defun +magit-worktree-register (directory &rest _)
    "Remember a newly created worktree DIRECTORY as a project.
Advises Magit's worktree-creation commands."
    (when-let* ((project (project-current nil directory)))
      (project-remember-project project)))
  (defun +magit-worktree-status (worktree)
    "Show WORKTREE's Magit status, switching to it as its project.
Overrides `magit-worktree-status', inheriting its interactive spec."
  ...
  :config
  (advice-add 'magit-worktree-checkout :after #'+magit-worktree-register)
  (advice-add 'magit-worktree-branch   :after #'+magit-worktree-register)
  (advice-add 'magit-worktree-status :override #'+magit-worktree-status)
...
(use-package claude-code-ide
  ...
  :preface
  (defun +claude-code-ide-project-buffer-name (directory)
    "Name the Claude Code buffer for DIRECTORY after its `project.el' name.
Unlike the default (directory basename), this respects `project-vc-name'
set in .dir-locals.el, so worktree sessions get distinct names like
\"*claude-code[project[worktree]]*\"."
  ...
  :init
  (setq claude-code-ide-buffer-name-function #'+claude-code-ide-project-buffer-name)
